...
`data/raw/`. Stage 2 is `analysis/processing/`, stage 3 is `analysis/plots/`.
...
```bash
julia --project=. -t auto analysis/sim_runs/<scenario>/<script>.jl
```
...
`sim_runs/<scenario>/` subdirectory that is **three** `dirname` calls to the root, **two**
to `analysis/`, and **three** `".."` segments to `data/`.
...
- **Each scenario owns its structs** (`analysis/helpers/types*.jl`), `include`d rather than
...
```bash
julia --project=. analysis/sim_runs/selection_1/inventory_sel1.jl
julia --project=. analysis/sim_runs/selection_2/inventory_sel2.jl
```
