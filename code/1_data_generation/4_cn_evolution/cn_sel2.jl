...
const OUT = joinpath(DATA, "CN_subsampled",  "selection_2")

# Stage 4 for selection scenario 2: gamma model only for this first pass (markov
# and deterministic deferred — see the design doc). sel2's gamma grid only ever
...
for N in (1_000, 10_000), d in (0.0, 0.5, 0.9), s in S_VALUES
    ...
    raw_path = joinpath(RAW, "gamma", "sel2_gamma_N$(N)_d$(d)_k5.0_s$(s)_M$(M_CAP)_n$(n).jls")
