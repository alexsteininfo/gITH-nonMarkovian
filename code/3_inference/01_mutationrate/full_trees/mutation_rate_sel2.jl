# Mutation-rate inference on full sel_2 trees.
#
# For each shard in data/raw/selection_2/<timing>/*.jls, deserialize the trees,
...
# to data/inference/mutationrate/selection_2/full_trees/<stem>.jls with atomic
# writes and shard-level resumability.
#
# Inputs:  data/raw/selection_2/{deterministic,gamma,markov}/*.jls
# Outputs: data/inference/mutationrate/selection_2/full_trees/<stem>.jls
...
#          :n_restarts (Int from the source simulation)
#
# Usage:
#   julia --project=. -t auto code/3_inference/01_mutationrate/full_trees/mutation_rate_sel2.jl
#
...
const IN_ROOT  = joinpath(DATA, "raw", "selection_2")
const OUT_ROOT = joinpath(DATA, "inference", "mutationrate", "selection_2", "full_trees")
mkpath(OUT_ROOT)
...
function main()
    ...
    println("Found $(length(shards)) sel_2 full-tree shards.")
    for path in shards
        process_shard(path)
    end
end
