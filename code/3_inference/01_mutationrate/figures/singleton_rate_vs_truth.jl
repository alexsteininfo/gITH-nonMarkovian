# Figure 4: singleton_rate median per shard vs. truth (m = 2).
#
# Reads every data/inference/mutationrate/neutral/full_trees/*.jls shard,
# computes the per-shard median of singleton_rate, and draws a scatter plot
# against M_TRUE = 2.0, colour by k, marker shape by d, with a y = x diagonal.
#
# The singleton estimator is expected to be well-calibrated for Markov but to
# lie systematically below the diagonal for gamma/deterministic — showing this
# offset is the figure's message.
#
# Inputs:  data/inference/mutationrate/neutral/full_trees/*.jls
# Outputs: figures/3_inference/mutationrate/singleton_rate_vs_truth.png
#
# Usage:
#   julia --project=. code/3_inference/01_mutationrate/figures/singleton_rate_vs_truth.jl
...
function main()
    ...
    Label(fig[0, 1:2], "singleton_rate median vs. truth  (m = $M_TRUE)";
          fontsize=FS_HEAD, halign=:center)
    ax = Axis(fig[1, 1];
              xlabel="truth (m = $M_TRUE)", xlabelsize=FS_LABEL,
              ylabel="m̂_singleton (median ± IQR)", ylabelsize=FS_LABEL,
    ...
    if isempty(rows)
        ...
        save(joinpath(OUT_DIR, "singleton_rate_vs_truth.png"), fig)
        println("wrote (empty) ", joinpath(OUT_DIR, "singleton_rate_vs_truth.png"))
        return
    end
    ...
    out = joinpath(OUT_DIR, "singleton_rate_vs_truth.png")
    save(out, fig)
    println("wrote ", out)
end
