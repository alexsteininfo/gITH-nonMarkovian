# Mutation-rate inference on subsampled neutral trees.
#
# For each shard in data/raw_subsampled/neutral/<timing>/*.jls, deserialize the
# subsampled trees, convert to EvoTree twice (primary = :mutations, :divisions),
# run the full mutation-rate statistic suite, and serialize a
# Vector{Union{Nothing,Dict}} to
# data/inference/mutationrate/neutral/subsampled_trees/<stem>.jls with atomic
# writes and shard-level resumability.
#
# Inputs:  data/raw_subsampled/neutral/{deterministic,gamma,markov}/*.jls
# Outputs: data/inference/mutationrate/neutral/subsampled_trees/<stem>.jls
...
#
# Usage:
#   julia --project=. -t auto code/3_inference/01_mutationrate/subsampled_trees/mutation_rate_neutral_subsampled.jl
#
...
const OUT_ROOT = joinpath(DATA, "inference", "mutationrate", "neutral", "subsampled_trees")
mkpath(OUT_ROOT)
...
function process_shard(path::String)
    ...
    Threads.@threads for i in 1:min(length(subs), MAX_SIMS_PER_SHARD)
        ...
        results[i] = Dict{Symbol,Any}(:mutations => d_mut, :divisions => d_div,
                                       :params => p)
    end
