...
function make_sfs_figure(;
    ...
    for (ni, N) in enumerate(n_values), (di, d) in enumerate(d_values)
        ...
        if !isfile(fpath)
            ax = Axis(fig[ni + 1, di];
                ...
                titlesize = FS_TITLE, xscale = log10, yscale = log10, spinewidth = 0.8)
        ...
        if !isempty(valid_th)
            lines!(ax, Float64.(k_range[valid_th]), theory[valid_th];
                   color = COL_THEORY_MARKOV, linewidth = 1.4, linestyle = :dash)
        end
        ...
        if model == "deterministic"
            ...
            lines!(ax, Float64.(k_det), sfs_det;
                   color = COL_THEORY_DET, linewidth = 1.4, linestyle = :dot)
        end
    end
    ...
    leg_elems = LegendElement[
        ...
        LineElement(color = COL_THEORY_MARKOV, linewidth = 1.4, linestyle = :dash),
    ]
    leg_labels = ["mean ± std", "mean", "single simulation", "Markovian theory (Eq. 9)"]
    if model == "deterministic"
        push!(leg_elems,  LineElement(color = COL_THEORY_DET, linewidth = 1.4, linestyle = :dot))
