...
function draw_scMB_panel!(
    ...
    scatter!(ax, js, single;       color = col_single, markersize = 4)
    lines!(ax, js, hmean;          color = col_mean,          linewidth = 4.0)
    lines!(ax, js, theory_m;       color = COL_THEORY_MARKOV, linewidth = 2.5, linestyle = :dash)
    lines!(ax, js, theory_d;       color = COL_THEORY_DET,    linewidth = 5.0, linestyle = :dot)
...
function make_scMB_figure(;
    ...
    for (ni, N) in enumerate(n_values), (di, d) in enumerate(d_values)
        ...
        if !isfile(fpath)
            ax = Axis(fig[ni + 1, di];
                ...
                titlesize = FS_TITLE, spinewidth = 1.3)
    ...
    Legend(fig[nrows + 2, 1:ncols],
        [
            ...
            LineElement(color = COL_THEORY_DET,    linewidth = 5.0, linestyle = :dot),
        ],
